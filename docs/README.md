...
- **多源知识融合**
  - 医疗知识图谱：疾病–症状–药品–科室等实体关系（Neo4j）
  - 文本向量检索：病例、说明书、文档语料（Milvus）
  - 大模型问答：如 DeepSeek，用于生成最终回答

- **模块职责清晰**
  - `config/`：集中配置管理（环境变量 + 默认值）
  - `core/`：Embedding、LLM、向量库、缓存、知识图谱等基础能力
  - `services/`：Agent 服务、图谱服务等业务逻辑
  - `api/`：FastAPI 路由与中间件（可选扩展）
  - `web/`：前端聊天界面（静态 HTML）

- **本地化与可扩展**
  - `storage/models/`：预留本地模型权重目录，可扩展为纯本地推理
  - `storage/databases/`：本地 Milvus 数据文件
  - `data/`：集中管理原始数据、词典和处理后数据

- **工程化实践**
  - 一键启动脚本 `start.sh`
  - 模块内 `README.md` 文档说明
  - 日志、PID、数据库等均落在 `storage/` 下，便于管理

更细粒度的模块说明见各模块目录下的 `README.md`，以及 `docs/technical_workflow.md` 中的技术流程说明。
...
常用子模块文档入口（均为对应目录下的 `README.md`）：

- 配置模块：`config/README.md`
- 核心模块总览：`core/README.md`
  - 模型封装（LLM / Embedding）：`core/models/README.md`
  - 向量库（Milvus）：`core/vector_store/README.md`
  - 缓存（Redis）：`core/cache/README.md`
  - 知识图谱（Neo4j + NL2Cypher）：`core/graph/README.md`
- 服务层：`services/README.md`
- API 层：`api/README.md`
- 数据目录说明：`data/README.md`
- 存储目录说明：`storage/README.md`
- 工具脚本说明：`utils/README.md`
- 前端与启动脚本说明：`web/README.md`、`scripts/README.md`
- 整体技术流程：`docs/technical_workflow.md`
...
主要配置集中在 `config/settings.py`，支持从环境变量中读取：
...
建议通过系统环境变量配置敏感信息，**不推荐** 在代码中硬编码 API Key 和密码。
...
更详细的架构与调用链说明，见 `docs/technical_workflow.md`。
...
- 在 `docs/technical_workflow.md` 中补充技术流程
